...
router.post("/", CheckUser, async (req, res) => {
  ...
  try {
    ...
    const chatting = model.startChat({
      generationConfig,
      safetySettings,
      history: [],
    });

    const result = await chatting.sendMessage(prompt);
    response.openai = result.response;
    if (response?.openai?.text()) {
      response.openai = response.openai.text();
      response.db = await chat.newResponse(prompt, response, userId);
    }
    //
  } catch (err) {
